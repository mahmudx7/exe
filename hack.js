...
module.exports = {
  ...
  onStart: async function ({ args, api, event }) {
    ...
    try {
      ...
      if (event.mentions && Object.keys(event.mentions).length) targetId = Object.keys(event.mentions)[0];
      else if (event.messageReply && event.messageReply.senderID) targetId = event.messageReply.senderID;
      else if (args && args[0] && /^\d+$/.test(args[0])) targetId = args[0].trim();
      ...
      if (displayName === "Unknown" && event.mentions && event.mentions[targetId]) {
        displayName = event.mentions[targetId];
      }

      if (!displayName || displayName === "Unknown") displayName = targetId;
