...
module.exports = {
	...
	onStart: async function ({ message, event, usersData, commandName, envCommands, api }) {
		...
		const rankCards = await Promise.all(targetUsers.map(async userID => {
			const rankCard = await makeRankCard(userID, usersData, deltaNext, api);
...
async function makeRankCard(userID, usersData, deltaNext, api = global.GoatBot.fcaApi) {
	...
	// Removed customRankCard logic to simplify and remove dependency
	...
	const dataLevel = {
		...
		isVip: isVip // <-- PASS THE VIP STATUS
	};
...
class RankCard {
	...
	async buildCard() {
		...
		const width = Number(widthCard); // <-- ADDED
		const height = Number(heightCard); // <-- ADDED
	...
	try {
		...
		const bx = width - badgeSize - 300; // more left
