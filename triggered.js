...
module.exports = {
  config: {
    ...
    category: "fun",
    cooldown: 10,
    guide: "trigger [mention-reply-UID]",
  },
  ...
  onStart: async function ({ api, event, args }) {
    ...
    try {
      const url = `${await baseApiUrl()}/api/dig?type=trigger&user=${id2}`;
      const img = await axios.get(url, { responseType: "arraybuffer" });
      const file = path.join(__dirname, `trigger_${id2}.png`);
      fs.writeFileSync(file, img.data);

      api.sendMessage({
        body: "Effect trigger successful ⚡",
